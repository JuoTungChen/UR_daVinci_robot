...
class Pose:
    ...
    def __init__(self, translation=None, rotation=None):
        self.p = np.array([0, 0, 0]) if translation is None else np.asarray(translation)

        if rotation is None:
            self.q = np.quaternion(1, 0, 0, 0)
        elif isinstance(rotation, np.quaternion):
            self.q = rotation
        else:
            self.q = quaternion.as_quat_array(rotation)
